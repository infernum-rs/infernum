...
///
/// Weights are kept in packed Q4_0 format (16 bytes/block); input is pre-quantized
/// Q8_0 (32 bytes/block). Nibbles are unpacked inline inside the microkernel,
/// giving 2× smaller weight reads vs the expand-first path.
...
pub fn gemm_q4_tiled(
    ...
    // Scalar fallback: row-by-row Q4×Q8 dot products.
    for row in 0..m {
        let iq = &inp_quants[row * inp_bytes_per_row..(row + 1) * inp_bytes_per_row];
        let is_ = &inp_scales[row * num_blocks..(row + 1) * num_blocks];
        for col in 0..n {
            let wq = &wt_quants[col * wt_bytes_per_row..(col + 1) * wt_bytes_per_row];
            let ws = &wt_scales[col * num_blocks..(col + 1) * num_blocks];
            let mut sum = 0.0f32;
            for b in 0..num_blocks {
                let mut dot = 0i32;
                let packed = &wq[b * 16..(b + 1) * 16];
                let quants = &iq[b * 32..(b + 1) * 32];
                for k in 0..16 {
                    let lo = i32::from(packed[k] & 0x0F) - 8;
                    let hi = i32::from(packed[k] >> 4) - 8;
                    dot += lo * i32::from(quants[2 * k] as i8);
                    dot += hi * i32::from(quants[2 * k + 1] as i8);
                }
                sum += (dot as f32) * ws[b] * is_[b];
            }
            output[row * n_stride + col] = sum;
        }
    }
}
