...
// ---- Q4_0 × Q8_0 native GEMM (inline nibble unpacking) ----

/// Tiled Q4_0×Q8_0 GEMM with inline nibble unpacking.
///
/// Computes `output[m, n_stride] = inp_quants[m, k] · wt_quants[n, k]`
/// where `inp_quants` is Q8_0 (pre-quantized input, 32 bytes/block) and
/// `wt_quants` is Q4_0 packed (16 bytes/block). Scales for both are f32.
///
/// Processes weight data directly from packed 4-bit format — no expansion
/// to int8 is needed, giving 2× smaller weight reads vs the expand-first path.
#[allow(clippy::too_many_arguments, clippy::many_single_char_names)]
pub fn gemm_q4_tiled(
    output: &mut [f32],
    inp_quants: &[u8],
    inp_scales: &[f32],
    wt_quants: &[u8],
    ...
    n_stride: usize,
    num_blocks: usize,
    inp_bytes_per_row: usize,
    wt_bytes_per_row: usize,
) {
    unsafe {
        gemm_q4_tiled_inner(
            output,
...
unsafe fn gemm_q4_tiled_inner(
    ...
    for i in (0..m_full).step_by(4) {
        for j in (0..n_full).step_by(4) {
            microkernel_q4_4x4(
                output,
                inp_quants,
                inp_scales,
        ...
        for jj in n_full..n {
            for ii in i..i + 4 {
                output[ii * n_stride + jj] = dot_q4_q8_row(
                    &inp_quants[ii * inp_bytes_per_row..(ii + 1) * inp_bytes_per_row],
                    &inp_scales[ii * num_blocks..(ii + 1) * num_blocks],
                    &wt_quants[jj * wt_bytes_per_row..(jj + 1) * wt_bytes_per_row],
                    &wt_scales[jj * num_blocks..(jj + 1) * num_blocks],
                );
            }
        }
...
/// 4×4 Q4_0×Q8_0 micro-kernel with inline nibble unpacking using AVX-512 VNNI.
///
/// Input (`inp_quants`) is pre-quantized Q8_0 (32 signed int8 per block).
/// Weights (`wt_quants`) are Q4_0 packed (16 bytes per block, 32 nibbles).
///
/// Per block, per weight column: load 16 Q4 bytes, unpack via vpand/vpsrlw/
/// vpsubb/vpunpcklbw/vpunpckhbw/vinserti128 to 32 signed int8 values, then
/// apply the same vpsignb + vpdpbusd + scale path as microkernel_q8_4x4.
///
/// Register map:
/// - ymm0-ymm3:   4 input quant rows (Q8, 32 bytes each, reloaded per K block)
/// - ymm4:        weight quants after unpack (32 int8 from 16 Q4 bytes)
/// - ymm5:        w_abs = vpsignb(ymm4, ymm4)
/// - ymm6:        scratch (dpbusd accumulator, unpack intermediates)
/// - ymm7:        scratch (vpsignb result, vmulps scale product)
/// - ymm8-ymm23:  16 accumulators — col×4 + row for 4×4 tile
/// - ymm24-ymm27: pre-broadcast input scales (is0-is3, one per K block)
/// - ymm28:       weight scale broadcast (ws for current column)
/// - ymm29:       nibble mask constant {0x0F} × 32 (set once before loop)
/// - ymm30:       nibble bias constant {0x08} × 32 (set once before loop)
/// - ymm31:       unpack temp (vpunpcklbw result, overwritten per column)
///
/// GPR map (same roles as microkernel_q8_4x4):
/// - iq0-iq3:    input Q8 row base pointers
/// - is0-is3:    input scale row base pointers (cast to byte ptrs)
/// - arr_ptr:    pointer to ptrs[] (8 entries: 4 weight quant ptrs, 4 scale ptrs)
/// - q_off:      Q4 weight byte offset, increments by 16 per K block
/// - iq_off:     Q8 input byte offset = 2 * q_off (computed per iteration)
/// - s_off:      scale f32 byte offset = q_off >> 2 (computed per iteration)
/// - q_limit:    loop bound = num_blocks * 16
/// - nibble_mask_ptr, nibble_bias_ptr: byte addresses of 0x0F and 0x08 constants
/// - tmp:        scratch for ptr loads from arr_ptr
#[allow(
    clippy::too_many_arguments,
    clippy::too_many_lines,
    clippy::similar_names,
...
unsafe fn microkernel_q4_4x4(
    ...
    let iq0 = inp_quants.as_ptr().add(i * inp_bytes_per_row);
    let iq1 = inp_quants.as_ptr().add((i + 1) * inp_bytes_per_row);
    let iq2 = inp_quants.as_ptr().add((i + 2) * inp_bytes_per_row);
    let iq3 = inp_quants.as_ptr().add((i + 3) * inp_bytes_per_row);
    ...
    // ptrs[0..4) = weight quant row pointers (Q4 packed)
    // ptrs[4..8) = weight scale row pointers (f32)
    // ptrs[8]    = &nibble_mask (0x0F) — loaded into ymm29 before loop
    // ptrs[9]    = &nibble_bias (0x08) — loaded into ymm30 before loop
    let nibble_mask: u8 = 0x0F;
    let nibble_bias: u8 = 0x08;
    let ptrs: [*const u8; 10] = [
        wt_quants.as_ptr().add(j * wt_bytes_per_row),
        wt_quants.as_ptr().add((j + 1) * wt_bytes_per_row),
        wt_quants.as_ptr().add((j + 2) * wt_bytes_per_row),
        wt_quants.as_ptr().add((j + 3) * wt_bytes_per_row),
        wt_scales.as_ptr().add(j * num_blocks).cast(),
        wt_scales.as_ptr().add((j + 1) * num_blocks).cast(),
        wt_scales.as_ptr().add((j + 2) * num_blocks).cast(),
        wt_scales.as_ptr().add((j + 3) * num_blocks).cast(),
        &nibble_mask as *const u8,
        &nibble_bias as *const u8,
    ];

    let q_limit = num_blocks * 16; // Q4: 16 bytes per block
    ...
    if num_blocks > 0 {
        std::arch::asm!(
            ...
            // Broadcast nibble constants into ymm29 (mask 0x0F) and ymm30 (bias 0x08).
            // Uses {tmp}; ptrs[8] = &nibble_mask, ptrs[9] = &nibble_bias.
            "mov {tmp}, [{arr_ptr} + 8*8]",
            "vpbroadcastb ymm29, byte ptr [{tmp}]",
            "mov {tmp}, [{arr_ptr} + 9*8]",
            "vpbroadcastb ymm30, byte ptr [{tmp}]",

            // Loop counter (Q4 weight byte offset, increments by 16 per block).
            "xor {q_off:e}, {q_off:e}",

            "2:",
            // tmp = 2 * q_off  (Q8 input: 32 bytes per block = 2× Q4's 16).
            "lea {tmp}, [{q_off} + {q_off}]",
            // s_off = q_off >> 2  (f32 scale offset: 16 Q4 bytes / 4 bytes f32 = 4:1).
            "mov {s_off}, {q_off}",
            "shr {s_off}, 2",

            // Load 4 input quant rows (Q8, 32 bytes each) into ymm0-3.
            "vmovdqu ymm0, [{iq0} + {tmp}]",
            "vmovdqu ymm1, [{iq1} + {tmp}]",
            "vmovdqu ymm2, [{iq2} + {tmp}]",
            "vmovdqu ymm3, [{iq3} + {tmp}]",
            ...
            "vpxord  ymm6, ymm6, ymm6",
            "vpdpbusd ymm6, ymm5, ymm7",
            "mov {tmp}, [{arr_ptr} + 1*8]",  // early-load wq1 ptr
            "vcvtdq2ps ymm6, ymm6",
            "vmulps ymm7, ymm27, ymm28",
            "vfmadd231ps ymm11, ymm6, ymm7",
            ...
            "vpxord  ymm6, ymm6, ymm6",
            "vpdpbusd ymm6, ymm5, ymm7",
            "mov {tmp}, [{arr_ptr} + 2*8]",  // early-load wq2 ptr
            "vcvtdq2ps ymm6, ymm6",
            "vmulps ymm7, ymm27, ymm28",
            "vfmadd231ps ymm15, ymm6, ymm7",
            ...
            "vpxord  ymm6, ymm6, ymm6",
            "vpdpbusd ymm6, ymm5, ymm7",
            "mov {tmp}, [{arr_ptr} + 3*8]",  // early-load wq3 ptr
            "vcvtdq2ps ymm6, ymm6",
            "vmulps ymm7, ymm27, ymm28",
            "vfmadd231ps ymm19, ymm6, ymm7",
            ...
            // Advance block (16 bytes for Q4 weights).
            "add {q_off}, 16",
            "cmp {q_off}, {q_limit}",
            "jb 2b",

            // ---- Horizontal reduction ----
            // Same as microkernel_q8_4x4: reduce each ymm → scalar, store to out[].
    ...
    // Scatter 16 output scalars to the strided output matrix.
    // out layout: [col * 4 + row], output layout: row-major [row * n + col].
    let out_ptr = output.as_mut_ptr();
    for row in 0..4 {
        for col in 0..4 {
