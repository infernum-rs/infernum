//! Metal text generation example (multi-architecture, `SafeTensors` only)
//!
//! Usage:
...
/// Metal text generation with LLMs (`SafeTensors`)
///
/// Supports Llama, Mistral, Mixtral, Qwen, and Gemma model families.
/// Models are loaded as f32 via `SafeTensors`.
...
fn run_model<M>(model: M, tokenizer: LlamaTokenizer, cli: &Cli) -> Result<()>
where
