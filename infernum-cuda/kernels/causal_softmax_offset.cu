...
// For prefill with KV cache: query position i can attend to key positions
// [min_valid_k..offset + i + 1]. When window_size > 0, min_valid_k restricts
// attention to the most recent window_size positions.
...
extern "C" __global__ void causal_softmax_offset_f32(
    ...
    for (int i = min_valid_k + tid; i < max_valid_k && i < seq_k; i += blockDim.x) {
    ...
    for (int i = tid; i < seq_k; i += blockDim.x) {
        if (i >= min_valid_k && i < max_valid_k) {
