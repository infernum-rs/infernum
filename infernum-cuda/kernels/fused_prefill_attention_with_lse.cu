...
// Same as fused_prefill_attention but also outputs per-head log-sum-exp (LSE).
// LSE = log(sum(exp(score_i - max))) + max, which equals log(sum(exp(score_i))).
// This enables combining attention outputs from different key ranges via
// numerically-stable log-sum-exp correction (used in multi-chunk prefill).
//
// output: (seq_q, num_heads, head_dim)
// lse:    (seq_q, num_heads)

extern "C" __global__ void fused_prefill_attention_with_lse_f32(
    float* __restrict__ output,       // (seq_q, num_heads, head_dim)
    float* __restrict__ lse,          // (seq_q, num_heads) — log-sum-exp per head
    ...
    // Write LSE = log(sum_exp) + max (thread 0 only)
    if (tid == 0) {
        lse[qpos * num_heads + head] = logf(sum_val) + max_val;
    }
...
extern "C" __global__ void fused_prefill_attention_with_lse_bf16(
    __nv_bfloat16* __restrict__ output,
    float* __restrict__ lse,          // LSE is always f32
...
extern "C" __global__ void fused_prefill_attention_with_lse_f16(
