...
extern "C" __global__ void fused_decode_attention_f32(
    ...
    const int win_start = (window_size > 0) ? max(0, total_len - window_size) : 0;
    ...
    for (int t = win_start + tid; t < total_len; t += blockDim.x) {
        float dot = 0.0f;
    ...
    // Pass 3: Weighted output (only over windowed positions)
    for (int d = tid; d < head_dim; d += blockDim.x) {
        float acc = 0.0f;
        for (int t = win_start; t < total_len; t++) {
