...
fn generate_greedy(model_dir: &PathBuf, prompt: &str, max_tokens: usize) -> String {
    ...
    let model =
        LlamaModel::<MetalBackend>::from_pretrained(&ctx, model_dir).expect("Failed to load model");
...
mod smollm2_360m {
    ...
    fn no_nan_in_output() {
        ...
        let ctx = MetalContext::new();
        let model = LlamaModel::<MetalBackend>::from_pretrained(&ctx, &model_dir)
        ...
        let logits_vec = MetalBackend::to_f32_vec(&hidden).unwrap();
