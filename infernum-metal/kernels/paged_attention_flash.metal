...
// ---------------------------------------------------------------------------
// Q-f32 / KV-f16 variant: activations in float, KV cache in half.
// Reads 2x less KV bandwidth than the pure-f32 variant.
// ---------------------------------------------------------------------------
kernel void paged_attention_flash_decode_f32_kv16(
    device const float*  Q            [[buffer(0)]],
    device const half*   K_pool       [[buffer(1)]],
    device const half*   V_pool       [[buffer(2)]],
    device const int*    block_tables [[buffer(3)]],
