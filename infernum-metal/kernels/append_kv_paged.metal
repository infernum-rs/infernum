...
/// Fused K+V paged append: write both K and V in a single dispatch.
///
/// Thread grid: (num_kv_heads * head_dim, batch_size, 2)
///   z=0 → write K, z=1 → write V.
kernel void append_kv_paged_batched_fused_f32(
    device float*       k_pool     [[buffer(0)]],
    device float*       v_pool     [[buffer(1)]],
    device const float* k_data     [[buffer(2)]],
    device const float* v_data     [[buffer(3)]],
    ...
    uint3 tid [[thread_position_in_grid]])
{
    ...
    device float* pool = (kv_idx == 0) ? k_pool : v_pool;
    device const float* data = (kv_idx == 0) ? k_data : v_data;
    pool[dst] = data[src];
}


/// Fused K+V paged append: f32 input → f16 pool (down-converts on write).
/// Used when activations are f32 but the KV cache is stored in f16.
kernel void append_kv_paged_batched_fused_f32_to_f16(
