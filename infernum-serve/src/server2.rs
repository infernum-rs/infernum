...
//!
//! Provides the [`Server2`] builder for registering models and running
...
use infernum_runtime::{
    BatchConfig2 as BatchConfig, Engine2, FinishReason2 as FinishReason,
    GenerationEvent2 as GenerationEvent, TokenSender2 as TokenSender,
};
...
impl<M: Model> ErasedEngine for Engine2<M> {
    fn submit(
        &self,
        input_ids: Vec<u32>,
        ...
        Engine2::submit(self, input_ids, options, token_tx);
    }

    fn model_config(&self) -> &ModelConfig {
        Engine2::model_config(self)
    }
}
...
// ---------------------------------------------------------------------------
// ModelEntry2 — user-facing model registration
// ---------------------------------------------------------------------------
...
impl ModelEntry2 {
        ...
        C: ChatTemplate + 'static,
    {
        ...
        let engine = Engine2::new(model).expect("Failed to create engine");
        Self {
            name: name.to_string(),
            handle: ModelHandle {
    ...
    {
        let model_config = model.config();
        let engine = Engine2::with_config(model, batch_config).expect("Failed to create engine");
        Self {
            name: name.to_string(),
            handle: ModelHandle {
                engine: Box::new(engine),
...
/// Builder for constructing a [`Server2`].
...
impl ServerBuilder2 {
    ...
    #[must_use]
    pub fn add_model(mut self, entry: ModelEntry2) -> Self {
