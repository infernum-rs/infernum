...
impl LlamaModel<f32> {
    ...
    /// Run forward pass without KV cache and return logits.
    ///
    /// This method is only available for `LlamaModel<f32>` because the
    /// decomposed attention path (used without KV cache) requires f32 tensors.
    ///
    /// # Arguments
